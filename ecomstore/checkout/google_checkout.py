...
def _build_xml_shopping_cart(request):
    ...
    for cart_item in cart_items:
        ...
        item_description_text = doc.createTextNode(str(cart_item.full_name))
        item_description.appendChild(item_description_text)
        item.appendChild(item_description)

        """ need to calculate the charge for some included options """
        ...
        unit_price_text = doc.createTextNode(str(cart_item.price + extra_price))
    ...
    if discount != float('0.0'):
        item = doc.createElement('item')
        items.appendChild(item)

        item_name = doc.createElement('item-name')
        item_name_text = doc.createTextNode('Promotion')
        item_name.appendChild(item_name_text)
        item.appendChild(item_name)

        item_description = doc.createElement('item-description')
        item_description_text = doc.createTextNode(request.session.get('promotion_code','') + '|' + request.session['promotion_desc'])
        item_description.appendChild(item_description_text)
        item.appendChild(item_description)

        unit_price = doc.createElement('unit-price')
        unit_price.setAttribute('currency','USD')
        unit_price_text = doc.createTextNode('-' + str(request.session.get('discount', '')))
        unit_price.appendChild(unit_price_text)
        item.appendChild(unit_price)

        quantity = doc.createElement('quantity')
        quantity_text = doc.createTextNode(str('1'))
        quantity.appendChild(quantity_text)
        item.appendChild(quantity)

    isItGift = request.session.get('isItGift', 'No')
    if isItGift == 'Yes':
        item = doc.createElement('item')
        items.appendChild(item)

        item_name = doc.createElement('item-name')
        item_name_text = doc.createTextNode('Gift purchase - Price printed ' + '|' + request.session['pricePrinted'])
        item_name.appendChild(item_name_text)
        item.appendChild(item_name)

        item_description = doc.createElement('item-description')
        item_description_text = doc.createTextNode('Gift msg - ' + request.session['giftmessage'])
        item_description.appendChild(item_description_text)
        item.appendChild(item_description)

        unit_price = doc.createElement('unit-price')
        unit_price.setAttribute('currency','USD')
        unit_price_text = doc.createTextNode('0')
        unit_price.appendChild(unit_price_text)
        item.appendChild(unit_price)
    ...
    if note:
        ...
        item_name = doc.createElement('item-name')
        item_name_text = doc.createTextNode('Note for the order ')
        item_name.appendChild(item_name_text)
        item.appendChild(item_name)

        item_description = doc.createElement('item-description')
        item_description_text = doc.createTextNode(note)
        item_description.appendChild(item_description_text)
        item.appendChild(item_description)
    ...
    shipping_method_name = request.session.get('shippingLevel','Standard Delivery')
    shippingmethod = ShippingMethod.objects.filter(name=shipping_method_name)[0]
    shipping_charge = cart.shipping_handling(request, shippingmethod.shipping_rate)
    ...
    flat_rate_shipping.setAttribute('name',shipping_method_name)
    ...
    shipping_price_text = doc.createTextNode(str(shipping_charge))
    shipping_price.appendChild(shipping_price_text)

    return_url = doc.createElement('continue-shopping-url')
    merchant_flow.appendChild(return_url)
    return_url_txt = doc.createTextNode(settings.SITE_URL)
    return_url.appendChild(return_url_txt)
