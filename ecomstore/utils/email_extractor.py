...
image_type = [".flv", ".cqs",".zip",".jpg", ".jpeg", ".png", ".gif", ".pdf", ".mp4", ".wmv",".mov",".wma",".mid", ".midi", ".rmi", ".avi",".aif", ".aifc", ".aiff",".mpg", ".mpeg", ".m1v", ".mp2", ".mp3", ".mpa", ".mpe", ".m3u"]

def is_image(url):
    if ".." in url:
        print "The url contains .., ignore"
        return True
    for i in image_type:
        if url.lower().endswith(i.lower()):
            try:
                print "it is an {} image, discard, {}".format(i, url)
            except:
                pass
            return True
    return False
...
class MyHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        if len(self.pageMap) >= self.maxUrls:
            return
        if (tag.upper() == "BASE"):
	    if len(attrs) > 0 and (attrs[0][0].upper() == "HREF"):
		self.baseUrl = joinUrls(self.baseUrl, attrs[0][1])
        if (tag.upper() == "A"):
            url = ""
	    for attr in attrs:
             try:
                if (attr[0] is None) or (attr[1] is None):
                    continue
            ...
            try:
                ...
                return
            if not(self.pageMap.has_key(url) and not(is_image(url))):
                self.pageMap[url] = ()
...
def parsePages(startUrl, maxUrls, blockExtensions):
    ...
    while True:
        ...
        if url == None:
            break
        try:
            print " {}".format(url.encode('utf-8'))
        except:
            print "run into a URL unicode exception, ignore"
...
def grab_email(text):
    ...
    #mailsrch = re.compile(("([a-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+\/=?^_`"
    ...
    u = {}
    for item in found:
        if not re.match(r"[^@]+@[^@]+\.[^@]+", item[0]):
            continue
        u[item[0]] = 1

    return u.keys()

def grab_water_testing_keywords(text):
    found = []
    #mailsrch = re.compile(r'[\w\-][\w\-\.]+@[\w\-][\w\-\.]+[a-zA-Z]{1,4}')
    #mailsrch = re.compile(("([a-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+\/=?^_`"
    #                "{|}~-]+)*(@|\sat\s)(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?(\.|"
    #                "\sdot\s))+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)"))
    mailsrch = re.compile('water test|Colilert|Colisure|Microbiology test|Microbiology Consult|Microbial test|Microbiology Service|microbiology media|sodium thiosulfate|Colitag|Enzyme Substrate Test|E.coli|E. Coli|coliform', re.IGNORECASE)
...
def get_emails(url, limit):
    emails = defaultdict(int)
    returns = []
    if not url.startswith("http://"):
        url = "http://" + url
    pageMap = crawl_site(url, limit)
    returns.append("@"+str(len(pageMap)))
    for url in pageMap:
        for email in grab_email(urltext(url)):
            if not emails.has_key(email):
                print email
                returns.append(email)
            emails[email] += 1
    return returns

def get_water_sites(url, limit):
    emails = defaultdict(int)
    returns = []
    if not url.startswith("http://"):
        url = "http://" + url
    ...
    #returns.append("@"+str(len(pageMap)))
    for url in pageMap:
        for email in grab_water_testing_keywords(urltext(url)):
            if not emails.has_key(email):
                print email
                returns.append(email)
            emails[email] += 1
