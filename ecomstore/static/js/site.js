		...
		init : {
			...
			paginationSortCompare : function() {
				...
				$(".js-compareCheckedItems").bind("click.compare", function(evt) {
					evt.preventDefault();
					var prodRegEx = new RegExp("entityId", "g");
					var params = $(".js-compareCheckbox").serialize().replace(prodRegEx, "productId");
					if( params == "" ) {
						$('#top-errors').append('<div class="errorNotification errorText">There are currently no items selected to compare.</div>');
					}
					else {
...
$(function() {
	...
	/* Header Search Field */
	lib.input.defaultText(".js-searchInput", { defaultText : "keyword or SKU number" });
