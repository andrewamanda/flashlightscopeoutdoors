...
from dinette.models import Ftopics , SuperCategory ,Category ,Reply, DinetteUserProfile, get_user_profile
...
def index_page(request):
    ...
    # create dinette user profile record if this is the first visit
    ...
    # Generate a list of groups for a user for debugging
#    s_groups=""
#    for group in groups:
#      s_groups = s_groups + ":" + str(group)
#    print "dinette/index_page: user '"+request.user.username+"' groups are "+s_groups
...
def postTopic(request) :
    ...
    if (datetime.now() -(get_user_profile(request.user).last_posttime)).seconds <= settings.FLOOD_TIME :
...
def postReply(request) :
    ...
    #print "postReply: has request=",request
