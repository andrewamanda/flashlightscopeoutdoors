...
def  get_user_profile(user):
  if user.is_authenticated():
    try:
      p=DinetteUserProfile.objects.get(user=user)
    except:
      p=DinetteUserProfile( )
      p.user=user
      p.last_activity=datetime.datetime.now()
      p.last_session_activity=datetime.datetime.now()
      p.save( )
      
      g=Group.objects.get(name='can_access_forums')
      user.groups.add(g)
      user.save( )
    
#    s_groups=""
#    for g in user.groups.all( ):
#      s_groups=s_groups+" "+str(g)
#    print "user '"+user.username+"' is in "+s_groups
    return p
  else:
    return None
