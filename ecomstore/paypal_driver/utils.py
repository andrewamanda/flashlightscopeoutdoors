...
from ecomstore.paypal_driver.models import PayPalResponse, PayPalResponseStatus

def process_payment_request(request, amount, currency, token, payerid):
    """
    ...
    driver = PayPal()
    result = driver.DoExpressCheckoutPayment(request, currency = currency, amount = str(amount), token = token, payerid = payerid)
